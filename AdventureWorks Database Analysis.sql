...
-- 3) Stored procedure to update the employees hire information.
...
-- 5) Stored procedure to update the employees personal information.
...
-- 6) Stored procedure to update the availble VacationHours and SickLeaveHours for employees.
...
execute Insert_Into_PersonPerson 292, 'EM', 0, 'Mr.', 'Josh', null, 'Sequeira', null, 2
GO

-- Views
-- 8) View to return all the employees personal details and addresses.
...
-- 9) View to return the employees department history.
...
-- 10) The total number of Male and Female employees, along with the total number of single males, single females, married males, married females.
...
-- 16) Using pivot table to find the number of employees hired each month for all the years.
with cte as
...
-- 17) Using pivot table to find number of employees under each salesYTD range based on TerritoryID.
with cte as
...
GO
-- 1) Function to find number of employees in each deprtment.
...
GO
-- 2) Find number of employees based on the shiftID value passed.
...
-- 3) Find list of employees based on the shiftIF value passed.
GO
...
-- Finding the average time it takes for an order to be shipped after it has been ordered.
...
-- Total number of orders placed in each month and year.
...
-- Total number of orders placed by sales executive Vs. customers online.
...
--  Total sales each year for all territories.
...
-- Total revenue generated from each store.
...
-- Number of products sold for each sales reason along with the revenue generated for each sales reason.
...
-- Views
-- Individual customers details that purchase Adventure Works Cycles products online.
...
-- Using PIVOT to return aggregated sales revenue generated by each sales employee.
...
-- List of all products which are manufactured in house, along with the sub category name and product category.
...
-- List of all products which are purchased, along with the sub category name and product category.
select p.ProductNumber, pc.name as ProductCategoryName, ps.name as ProductSubCategoryName, p.Name, 
...
-- Total number of products which are purchased Vs those which are manufactured in house.
with cte as 
...
-- List all the products names and product sub categories which come under the product 'Bikes' and mention which of these products are salable and which are not.
...
-- List all the products names and product sub categories which come under the product 'Components' and mention which of these products are salable and which are not.
select p.ProductID, pc.name as ProductCategory, p.Name as ProductName, ps.Name as ProductSubcategoryName, 
	   case when p.FinishedGoodsFlag = 0 then 'Product is not a salable item.'
			when p.FinishedGoodsFlag = 1 then 'Product is salable.' end ProductStatus
from [Production].[Product] p
...
-- List all the products names and product sub categories which come under the product 'Clothing' and mention which of these products are salable and which are not.
select p.ProductID, pc.name as ProductCategory, p.Name as ProductName, ps.Name as ProductSubcategoryName, 
	   case when p.FinishedGoodsFlag = 0 then 'Product is not a salable item.'
			when p.FinishedGoodsFlag = 1 then 'Product is salable.' end ProductStatus
from [Production].[Product] p
...
-- List all the products names and product sub categories which come under the product 'Accessories' and mention which of these products are salable and which are not.
select p.ProductID, pc.name as ProductCategory, p.Name as ProductName, ps.Name as ProductSubcategoryName, 
	   case when p.FinishedGoodsFlag = 0 then 'Product is not a salable item.'
			when p.FinishedGoodsFlag = 1 then 'Product is salable.' end ProductStatus
from [Production].[Product] p
...
-- Analysis of product cost and price. Using data only from FinishedGoodsFlag = 1.
-- Considering the cost of the product, find the profit which will be made for each product. Also generate a table with products with the top 10 most profitable products based on rank.
with cte as
...
-- List of all mountain products for women which are still on sale.
...
-- Using Functions. 3) Multi-Statement Table Functions
-- Using multi statement table function to find the products which are purchased vs those which are manufactured in house.
...
-- Total quantity of products ordered Vs quantity actually delivered from vendors.
...
-- Average amount spent to purchase products from vendors
select avg(TotalDue) as AvgCartValue
from [Purchasing].[PurchaseOrderHeader]
GO

-- List all vendors with creditrating 1, preferredVendorStatus 1, from USA and Canada
...
-- List total number of vendors from each country.
...
-- Which shipping companies made the most number of shipments from 2012 to 2013.
